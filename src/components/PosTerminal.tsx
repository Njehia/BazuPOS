...
export const PosTerminal: React.FC<PosTerminalProps> = ({ currentUser, onLogout }) => {
  ...
  const renderCartContent = (isDrawer: boolean) => (
    <div className="flex flex-col h-full bg-white">
      {/* Cart Header */}
      <div className="p-3.5 sm:p-4 border-b border-slate-100 flex justify-between items-center bg-white shrink-0">
        <div className="flex items-center gap-2">
          <h2 className="font-bold text-base sm:text-lg text-slate-900 flex items-center gap-2">
            <ShoppingBag className="w-5 h-5 text-amber-500" />
            <span>Current Cart</span>
            <span className="text-xs sm:text-sm font-normal text-slate-400">({totalItemsCount} items)</span>
          </h2>
        </div>
        ...
        <div className="flex items-center gap-2">
          {cart.length > 0 && (
            <button
              type="button"
              onClick={clearCart}
              className="text-xs text-red-500 font-bold border border-red-200 px-2 py-1 rounded hover:bg-red-50 cursor-pointer transition-colors"
            >
              CLEAR
            </button>
          )}
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3">
        {cart.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-400 space-y-2">
            ...
            <p className="text-xs text-slate-400">Click any product in the catalog to add it to the order.</p>
          </div>
        ) : (
          cart.map(({ product, quantity }, cartIdx) => {
            ...
            return (
              <div key={`cart-row-${product.id}-${cartIdx}`} className="flex gap-2.5 sm:gap-3 items-center bg-slate-50/70 p-2 sm:p-2.5 rounded-xl border border-slate-100">
                ...
                <div className="flex flex-col items-center gap-1 w-9 shrink-0">
                  <button
                    ...
                    className={`w-full h-7 flex items-center justify-center rounded text-slate-700 font-bold text-xs transition-colors ${
                      isAtMaxStock
                        ? 'bg-slate-100 text-slate-300 cursor-not-allowed'
                        : 'bg-white border border-slate-200 hover:bg-amber-50 hover:border-amber-300 cursor-pointer shadow-2xs'
                    }`}
                  >
                    +
                  </button>
                  <span className="text-xs font-bold py-0.5 text-slate-900 font-mono">
                    {quantity}
                  </span>
                  <button
                    type="button"
                    onClick={() => updateQuantity(product.id, -1)}
                    className="w-full h-7 flex items-center justify-center bg-white border border-slate-200 hover:bg-red-50 hover:border-red-200 rounded text-slate-700 font-bold text-xs cursor-pointer transition-colors shadow-2xs"
                  >
                    -
                  </button>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-xs sm:text-sm font-bold text-slate-800 uppercase leading-snug line-clamp-2">
                    {product.name}
                  </h4>
                ...
                <div className="text-right shrink-0">
                  ...
                  <button
                    ...
                    className="text-slate-400 hover:text-red-500 text-xs mt-1 p-1 transition-colors cursor-pointer"
      ...
      <div className="p-4 sm:p-5 bg-slate-50 border-t border-slate-200 space-y-3 shrink-0">
        <div className="flex justify-between items-center text-xs sm:text-sm">
          ...
          <span className="font-bold text-slate-900 font-mono">KES {totalAmount.toLocaleString()}</span>
        </div>
        <div className="flex justify-between items-center text-xs sm:text-sm">
          <span className="text-slate-500 font-medium">Tax (VAT 16%)</span>
        ...
        <div className="pt-2.5 border-t border-slate-200 flex justify-between items-end">
          <span className="text-sm sm:text-base font-bold text-slate-900 uppercase">Total</span>
          <span className="text-xl sm:text-2xl font-black text-slate-900 leading-none font-mono">
            <span className="text-xs align-top mr-1 font-normal text-slate-500">KES</span>
        ...
        <div className="grid grid-cols-2 gap-2.5 pt-1">
          <button
            ...
            className="bg-emerald-600 text-white rounded-xl py-3 px-2 flex flex-col items-center justify-center hover:bg-emerald-700 transition-all shadow-sm active:scale-98 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
          >
          ...
          <button
            ...
            className="bg-blue-600 text-white rounded-xl py-3 px-2 flex flex-col items-center justify-center hover:bg-blue-700 transition-all shadow-sm active:scale-98 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
          >
            <span className="text-[10px] font-bold opacity-80 uppercase tracking-wider">Payment</span>
            <span className="text-sm sm:text-base font-black tracking-tight flex items-center gap-1">
    ...
    <div className="flex flex-col h-screen w-screen bg-[#F8FAFC] font-sans text-slate-800 overflow-hidden select-none">
      ...
      <header className="h-16 bg-[#1E1B4B] text-white flex items-center justify-between px-4 sm:px-6 shadow-md shrink-0">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          {/* Flutter Architecture / Code Modal Button */}
          <button
            type="button"
            ...
            title="Inspect Flutter Folder Structure & DatabaseHelper.dart"
          >
          ...
          {/* Cashier Avatar Pill */}
          <div className="h-10 w-10 bg-slate-700 rounded-full flex items-center justify-center border border-slate-500 shrink-0 shadow-inner">
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0 border-r border-slate-200 bg-[#F8FAFC]">
          {/* Category Filter Bar */}
          <div className="p-3 sm:p-4 bg-white border-b border-slate-200 flex gap-2 overflow-x-auto no-scrollbar items-center shrink-0">
            ...
            {currentUser.role === 'ADMIN' && (
              <button
                ...
                className="px-3 py-2 rounded-xl text-xs whitespace-nowrap bg-amber-50 hover:bg-amber-100 text-amber-900 font-bold border border-dashed border-amber-300 transition-all cursor-pointer flex items-center gap-1.5 shrink-0"
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {filteredProducts.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-400 text-xs space-y-2 py-12">
                <Package className="w-12 h-12 text-slate-300" />
                <p className="text-sm font-medium text-slate-600">No products match your search or filter.</p>
                ...
                {filteredProducts.map((product, prodIdx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`bg-white border rounded-xl p-3 shadow-sm flex flex-col justify-between transition-all select-none ${
                        isOutOfStock
                          ? 'border-slate-200 opacity-60 cursor-not-allowed bg-slate-50/50'
                          : inCartQty > 0
                          ? 'border-amber-400 ring-1 ring-amber-400 cursor-pointer shadow-md'
                          : 'border-slate-200 hover:border-amber-400 cursor-pointer hover:shadow-md'
                      }`}
                    >
                      <div>
                        <div
                          className={`text-xs font-bold mb-1 uppercase tracking-tight ${
                            isOutOfStock ? 'text-red-600 tracking-tighter' : 'text-amber-600'
                          }`}
                        >
                          {isOutOfStock ? 'Out of Stock' : product.category}
                        </div>
                        <h3 className="text-sm font-bold leading-tight text-slate-800 line-clamp-2 min-h-[36px]">
                          {product.name}
                        </h3>
                      </div>
                        ...
                        <div className="flex justify-between items-end">
                          <span
                            className={`text-xs font-medium italic ${
                              isOutOfStock
                                ? 'text-red-500 font-bold uppercase not-italic'
                                : isLowStock
                                ? 'text-amber-600'
                                : 'text-slate-400'
                            }`}
          ...
          <div className="mt-auto p-4 bg-slate-900 flex flex-wrap sm:flex-nowrap items-center gap-4 text-white shrink-0 border-t border-slate-800">
        ...
        <div className="hidden lg:flex w-80 xl:w-96 flex-col bg-white shadow-[-10px_0_15px_-3px_rgba(0,0,0,0.05)] border-l border-slate-200 shrink-0">
      ...
      {cart.length > 0 && !isMobileCartOpen && (
        <div className="lg:hidden px-3 py-2.5 bg-slate-900 text-white border-t border-slate-800 shadow-xl flex items-center justify-between gap-3 shrink-0 z-20">
          <button
            type="button"
            onClick={() => setIsMobileCartOpen(true)}
      ...
      {isMobileCartOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex justify-end">
          ...
          {/* Drawer Container */}
          <div className="relative w-full max-w-sm sm:max-w-md bg-white h-full shadow-2xl flex flex-col z-10 animate-in slide-in-from-right duration-200">
      ...
      <nav className="h-12 bg-white border-t border-slate-200 flex items-center justify-center gap-6 sm:gap-12 shrink-0 select-none">
        ...
        <div
          onClick={() => handleOpenAdmin('inventory')}
          className="flex items-center gap-2 text-slate-400 font-bold h-full px-4 text-xs tracking-widest cursor-pointer hover:text-slate-600 transition-colors"
        >
        ...
        <div
          onClick={() => setIsFlutterCodeOpen(true)}
          className="hidden md:flex items-center gap-2 text-slate-400 font-bold h-full px-4 text-xs tracking-widest cursor-pointer hover:text-slate-600 transition-colors"
        >
      ...
      {/* Flutter Code & Architecture Inspector Modal */}
      {isFlutterCodeOpen && (
        <FlutterCodeModal onClose={() => setIsFlutterCodeOpen(false)} />
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-xs animate-fade-in">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 max-w-xs w-full shadow-2xl space-y-4">
            <div className="text-center">
              <div className="w-10 h-10 rounded-full bg-amber-500/15 text-amber-600 mx-auto flex items-center justify-center mb-2">
                <Shield className="w-5 h-5" />
              </div>
              <h3 className="text-sm font-bold text-slate-900">Administrator Access Required</h3>
            ...
            <form onSubmit={handleVerifyAdminPinPrompt} className="space-y-3">
              <input
                ...
                placeholder="••••"
                className="w-full text-center tracking-[0.5em] font-mono text-xl py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-slate-900 focus:outline-none focus:border-amber-500"
              />
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-white text-xs font-bold shadow-sm cursor-pointer transition-colors"
                >
                  Unlock
                </button>
              </div>
